...
TELEGRAM_BOT_TOKEN = '7678781870:AAEdEokUHXR7fzyULq5oY7-dNXt3tcdo6vw'
...
async def start(update: Update, context: CallbackContext):
    ...
    message = (
        "*🔥 𝗪𝗲𝗹𝗰𝗼𝗺𝗲 𝗧𝗼 𝗩𝗚 𝗖𝗵𝗲𝗮𝘁𝘀 𝗗𝗱𝗼𝘀*\n"
        "*🔥 𝗢𝘄𝗻𝗲𝗿 @seedhe_maut_bot*\n"
        "*🔥 𝗖𝗵𝗮𝗻𝗻𝗲𝗹 @seedhe_maut*\n"    
...
async def manage(update: Update, context: CallbackContext):
    ...
    if chat_id != ADMIN_USER_ID:
        await context.bot.send_message(chat_id=chat_id, text="*⚠️ 𝗬𝗼𝘂 𝗡𝗲𝗲𝗱 𝗧𝗼 𝗚𝗲𝘁 𝗣𝗲𝗿𝗺𝗶𝘀𝘀𝗼𝗻 𝗙𝗼𝗿 𝗨𝘀𝗲 𝗧𝗵𝗶𝘀 𝗖𝗼𝗺𝗺𝗮𝗻𝗱 𝗗𝗠 » @seedhe_maut_bot*", parse_mode='Markdown')
        return
...
async def attack(update: Update, context: CallbackContext):
    ...
    if user_id not in users:
        await context.bot.send_message(chat_id=chat_id, text="*🤡 𝗬𝗼𝘂 𝗡𝗲𝗲𝗱 𝗧𝗼 𝗚𝗲𝘁 𝗣𝗲𝗿𝗺𝗶𝘀𝘀𝗼𝗻 𝗧𝗼 𝗨𝘀𝗲 𝗕𝗼𝘁 𝗗𝗠 » @seedhe_maut_bot*", parse_mode='Markdown')
        return    

    if len(args) != 3:
        await context.bot.send_message(chat_id=chat_id, text="*🌟 𝗨𝘀𝗲𝘀 » /attack ip port time*", parse_mode='Markdown')
        return
    ...
    await context.bot.send_message(chat_id=chat_id, text=(
        ...
        f"*⭐ 𝗣𝗼𝗿𝘁 » {port}*\n"
        f"*⭐ 𝗧𝗶𝗺𝗲 » {time} seconds*\n"
        f"*🔥 𝗢𝘄𝗻𝗲𝗿 @seedhe_maut_bot*\n"        
        f"*🔥 𝗖𝗵𝗮𝗻𝗲𝗹 @seedhe_maut*"        
    ), parse_mode='Markdown')
